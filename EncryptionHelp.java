...
public class EncryptionHelp implements EventHandler<ActionEvent> {
	private GridPane _grid;
	private Text _text;

	public EncryptionHelp(GridPane grid, Text text) {
		_grid = grid;
		_text = text;
	}
	@Override
	public void handle(ActionEvent arg0) {
		...
		// Title
		Text title = new Text("Fix Profile");
		root.add(title, 0, 0, 2, 1);

		// User Name Label
		Label userName = new Label("User Name To Fix:");
		...
		btnCreate.setOnAction(
				new EncryptionHelpHandler(tfUserName, pfPassword, pfPasswordConf, primaryStage, _grid, _text));
